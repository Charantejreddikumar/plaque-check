...
def extract_teeth_roi(image: np.ndarray) -> tuple[np.ndarray, tuple[int, int, int, int]]:
    ...
    # 1. Tooth enamel region mask (bright, low saturation)
    tooth_lower = np.array([0, 0, 110], dtype=np.uint8)
    tooth_upper = np.array([179, 42, 255], dtype=np.uint8)
    tooth_mask = cv2.inRange(hsv, tooth_lower, tooth_upper)

    # 2. Oral mucosa / gum / lip mask (distinct red/pink tones around teeth)
    gum_mask_1 = cv2.inRange(hsv, np.array([0, 45, 30]), np.array([25, 255, 255]))
    gum_mask_2 = cv2.inRange(hsv, np.array([155, 45, 30]), np.array([180, 255, 255]))
    ...
    min_area = (width * height) * 0.003  # minimum 0.3% area of image
    valid_rects = []
    ...
    for cnt in contours:
        ...
        if area >= min_area:
            ...
            # Count tooth enamel pixels within this bounding box
            enamel_count = int(cv2.countNonZero(tooth_mask[y : y + h, x : x + w]))
            aspect_ratio = w / float(h)
            if 0.3 <= aspect_ratio <= 5.0 and enamel_count > 0:
                valid_rects.append((x, y, w, h, enamel_count))

    if not valid_rects:
        return image, (0, 0, width, height)

    # Select the rect with the highest concentration of tooth enamel
    ...
    # Only crop if the cropped ROI is at least 60x60 pixels
    if crop_w >= 60 and crop_h >= 60:
