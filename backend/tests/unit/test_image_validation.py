...
REJ_MATCH = "This is not a valid teeth image"


def test_validate_empty_image():
    with pytest.raises(ValueError, match=REJ_MATCH):
...
def test_validate_teeth_with_lips_and_skin_accepted():
    img = np.zeros((400, 400, 3), dtype=np.uint8)
    img[:, :] = (60, 60, 180)  # Reddish lips / gum background (B=60, G=60, R=180 -> HSV H~0, S~170, V~180)
    img[140:260, 80:320] = (220, 230, 240)  # Teeth enamel area
    # Add distinct vertical interdental gap lines
    for x in range(110, 300, 25):
        img[140:260, x:x+3] = (30, 30, 100)
    validate_teeth_image(img)


def test_validate_teeth_with_braces_accepted():
    img = np.zeros((400, 400, 3), dtype=np.uint8)
    img[:, :] = (60, 60, 180)  # Lips / gums background
    img[140:260, 80:320] = (220, 230, 240)  # Teeth enamel
    # Add metallic braces brackets and interdental gaps
    for x in range(110, 300, 25):
...
@pytest.mark.parametrize("skin_bgr", [
    (100, 120, 200),  # Fair skin tone with lip tint
    (70, 90, 170),    # Medium skin tone
    (40, 50, 120),    # Darker skin tone with lip context
    (60, 70, 190),    # Lips / reddish skin
])
def test_validate_varied_skin_tones_around_teeth(skin_bgr):
    img = np.zeros((400, 400, 3), dtype=np.uint8)
    img[:, :] = skin_bgr
    # Add oral cavity mucosa band behind teeth
    img[120:280, 80:320] = (50, 50, 170)
    img[150:250, 100:300] = (220, 230, 240)
    for x in range(120, 290, 25):
        img[150:250, x:x+3] = (30, 30, 90)
    validate_teeth_image(img)
...
def test_validate_different_lighting_conditions(lighting_bgr):
    img = np.zeros((400, 400, 3), dtype=np.uint8)
    img[:, :] = (60, 60, 170)  # Oral mucosa background
...
def test_validator_execution_time_under_100ms():
    ...
    for x in range(500, 1450, 80):
        img[350:750, x:x+8] = (30, 30, 100)
